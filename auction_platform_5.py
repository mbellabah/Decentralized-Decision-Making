...
class Transaction():
    ...
    def __init__(self, ask, bid, settled_price, settled_quantity):
        ...
        self.name = self.bid_ref + " " + self.ask_ref
...
# MARK: Agent Types - contains internal blockchain
...
class Commercial_Agent:
    def __init__(self, name, hasGen):
        ...
        self.blockchain = Blockchain(self.name)
        self.agent_main()
    ...
    def agent_main(self):
        ...
        # Agent Listener
        def transaction_listener(sender, transaction):
        ...
        # Agent Blockchain Listener
        def blockchain_order_listener(sender, order):
            order_type = order.get_type()
            print(
                "Order: {} placed of type: {} for price ${} and quantity {} from sender: Agent {}".format(
                    order,
                    order_type,
                    order.get_price(),
                    order.get_quantity(),
            ...
            self.blockchain.add_order(order)

            if order_type == "Bid":
                transaction = self.blockchain.match_order(order)
                if transaction:
                    self.blockchain.blockchain_confirm_transaction(sender.name, transaction)

        while 1:

            # Agent Listener Connection
            dispatcher.connect(
            ...
            # Agent Blockchain Listener Connection
            dispatcher.connect(
                blockchain_order_listener,
...
class Blockchain():
    def __init__(self, name):
        self.name = "blockchain"+name
        self.open_bids = {}
        self.open_asks = {}
        self.chain = []

        self.blockchain_main() # Listening to new orders sent by agents on network

    def blockchain_main(self):
        def blockchain_order_listener(sender, order):
            order_type = order.get_type()
            print(
                "Order: {} placed of type: {} for price ${} and quantity {} from sender: Agent {}".format(
            ...
            if order_type == "Bid":
                ...
                if transaction:
                    self.blockchain_confirm_transaction(sender.name, transaction)
            elif order_type == "Ask":
                pass
    ...
    def blockchain_confirm_transaction(self, sender_name, transaction):
        dispatcher.send(
            transaction=transaction,
            signal="Agent"+sender_name,
            sender= self.name
        )
        time.sleep(1)
